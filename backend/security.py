...
def hash_password(password: str) -> str:
    ...
    - Trim whitespace
    - Ensure minimum sanity length
    - Hash securely

    UX & strength validation belong to frontend.
    """
    ...
    # ✅ Match schemas.py & Flutter (min 6)
    if len(password) < 6:
...
def verify_access_token(token: str) -> Optional[str]:
    """
    Verify access token and return subject (email).
    """
    try:
        payload = jwt.decode(
            token,
...
def verify_refresh_token(token: str) -> Optional[str]:
    """
    Verify refresh token and return subject (email).
    """
    try:
        payload = jwt.decode(
            token,
